...
function ReportDetail({ report, t, onClose }) {
  const sc = STATE_COLORS[report.state] || STATE_COLORS.NOMINAL;
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 32 }}
      onClick={onClose}>
      <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: 16, width: "100%", maxWidth: 560, maxHeight: "80vh", overflow: "hidden", display: "flex", flexDirection: "column" }}
        onClick={e => e.stopPropagation()}>

        <div style={{ padding: "18px 24px", borderBottom: `1px solid ${t.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ fontSize: 14, fontWeight: 600, color: t.text }}>Crash Report Detail</div>
            <div style={{ fontSize: 11, color: t.textFaint, marginTop: 2, fontFamily: "'DM Mono', monospace" }}>{report.time} · cycle #{report.cycle}</div>
          </div>
          <button onClick={onClose} style={{ padding: "6px 14px", fontSize: 12, borderRadius: 8, border: `1px solid ${t.border}`, background: "transparent", color: t.textMuted, cursor: "pointer" }}>
            Close
          </button>
        </div>
        ...
        <div style={{ overflowY: "auto", padding: "20px 24px" }}>
          {/* State */}
          <div style={{ padding: "12px 16px", borderRadius: 10, marginBottom: 16, background: theme === "light" ? sc.bg : sc.darkBg, border: `1px solid ${sc.border}` }}>
            <div style={{ fontSize: 11, color: sc.color, fontWeight: 600, marginBottom: 4 }}>TRIGGER STATE</div>
            <div style={{ fontSize: 18, fontWeight: 600, color: sc.color }}>{report.state.replace("_"," ")}</div>
          </div>
          ...
          {/* Sensor readings */}
          <div style={{ fontSize: 12, fontWeight: 600, color: t.text, marginBottom: 10 }}>Sensor readings at fault</div>
              ...
              return (
                <div key={key} style={{ background: t.surfaceAlt, borderRadius: 6, padding: "8px 10px" }}>
                  ...
                  <div style={{ fontSize: 12, fontFamily: "'DM Mono', monospace", color: t.text }}>
                    {val != null ? val.toFixed(2) : "—"} <span style={{ fontSize: 9, color: t.textFaint }}>{meta.unit}</span>
                  </div>
                </div>
              );
            })}
          ...
          {report.chaos && (
            <div style={{ marginTop: 14, padding: "10px 14px", borderRadius: 8, background: "#d9770610", border: "1px solid #d9770625", fontSize: 12, color: "#d97706" }}>
              ⚡ Monte Carlo chaos was active at time of this event — readings may include injected noise.
            </div>
          )}
        </div>
      </div>
...
export default function PageCrashReports() {
  ...
  return (
    <div style={{ padding: "32px 36px" }}>
      {selected && <ReportDetail report={selected} t={t} theme={theme} onClose={() => setSelected(null)}/>}

      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 22, fontWeight: 600, color: t.text, letterSpacing: "-0.5px" }}>Crash Reports</h1>
        <p style={{ fontSize: 13, color: t.textMuted, marginTop: 4 }}>Black-box recorder — automatic fault event capture</p>
      </div>
      ...
      {crashReports.length === 0 ? (
        <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: 14, padding: "60px", textAlign: "center" }}>
          ...
          <div style={{ fontSize: 15, fontWeight: 500, color: t.text, marginBottom: 6 }}>No crash reports</div>
          <div style={{ fontSize: 13, color: t.textFaint }}>The black-box recorder will auto-capture events when faults are detected</div>
        </div>
      ) : (
              ...
              onMouseLeave={e => e.currentTarget.style.borderColor = t.border}>
                ...
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 10, padding: "1px 6px", borderRadius: 4, background: t.surfaceAlt, color: t.textFaint }}>{r.source}</span>
                  </div>
                  <div style={{ fontSize: 11, color: t.textFaint, fontFamily: "'DM Mono', monospace" }}>
                    {r.time} · cycle #{String(r.cycle).padStart(3,"0")} · key sensors: {r.topSensors?.join(", ") || "—"}
                  </div>
                </div>
                <div style={{ fontSize: 11, color: t.textFaint }}>View →</div>
              </div>
            );
          })}
        </div>
